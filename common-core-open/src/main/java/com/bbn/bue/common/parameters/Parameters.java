...
/**
 * Represents a set of parameters passed into a program.  The parameters are
 * assumed to originate as key-value pairs of <code>String</code>s, which
 * can then be accessed in various validated ways. This class is immutable. Keys will
 * never be null or empty. Values will never be null.
 *
 * For all methods to get parameters, looking up a missing parameter throws an unchecked
 * {@link MissingRequiredParameter} exception.
 *
 * @author rgabbard
 *
 */
...
public class Parameters {
	...
	/**
	 * Creates a new set of parameters with only those parameters in the specified namespace
	 * (that is, prefixed by "namespace.". The namespace prefix and period will be removed
	 * from parameter names in the new {@code Parameters}.
	...
	/**
	 * If the specified namespace is present, return a copy of that namespace
	 * as a parameter set. Otherwise, return a copy of this parameter set.
	 * @param requestedNamespace
	 * @return
	 */
	...
	/**
	 * Returns if any parameter in this parameter set begins the the specified
	 * string, followed by a dot. The argument may not be empty.
	 * @param requestedNamespace
	 * @return
	 */
	...
	/**
	 * Dumps the parameters object as colon-separated key-value pairs. If {@code printDateTime}
	 * is true, will put a #-style comment with the current date and time at the top.
	 * If includeNamespacePrefix is true, will prefix its parameter with its full namespace
	 * instead of writing all keys relative to the current namespace.
	 */
	...
	/**
	 * Gets the parameter string for the key <code>param</code>, then runs it
	 ...
	 * @return
	 */
	public <T> T get(final String param, final StringConverter<T> converter, final Validator<T> validator,
	 ...
	 * @return
	 */
	public <T> List<T> getList(final String param, final StringConverter<T> converter, final Validator<T> validator,
		final String expectation)
	{
		checkNotNull(param);
		checkNotNull(converter);
	...
	/**
	 * Looks up a parameter, then uses the value as a key in a map lookup.  If the value is not
	 * a key in the map, throws an exception.
	 *
	 * @param possibleValues May not be null. May not be empty.
	 * @throws InvalidEnumeratedPropertyException if the parameter value is not on the list.
	 */
	public <T> T getMapped(final String param, final Map<String, T> possibleValues) {
		...
		if (ret == null) {
			throw new InvalidEnumeratedPropertyException(fullString(param), value, possibleValues.keySet());
		}
		return ret;
	}
		...
		final Class<T> superClass)
{
	...
	private <T> T parameterInitializedObjectForClass(final Class<?> clazz,
		...
		if (superClass.isInstance(ret)) {
			...
			throw new ParameterValidationException(fullString(param), getString(param),
				new RuntimeException(String.format("Can't cast %s to %s", clazz.getName(), superClass.getName())));
		}
	}
	...
	/**
	 * Gets a directory which is guaranteed to exist after the execution of this
	 * method.  If the directory does not already exist, it and its parents
	 * are created. If this is not possible, an exception is throws.
	 */
	...
	public void assertAtLeastOneDefined(final String param1, final String param2) {
		if (!isPresent(param1) && !isPresent(param2)) {
			throw new ParameterException(String.format("At least one of %s and %s must be defined.", param1, param2));
		}
	}
	...
	public File getExistingFileRelativeTo(final File root, final String param) {
		if (!root.exists()) {
			throw new ParameterException(String.format("Cannot resolve parameter %s relative to non-existent directory", param), new FileNotFoundException(String.format("Not found: %s", root)));
		}
		final String val = getString(param);
		final File ret = new File(root, val);
		if (!ret.exists()) {
			throw new ParameterValidationException(fullString(param), ret.getAbsolutePath(), "Requested existing file, but the file does not exist");
		}
		return ret;
	}

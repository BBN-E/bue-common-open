...
public final class IterableUtils {
	 ...
	 * that returns true if an only if any the predicate is true for
	 * any of the iterable's contents.
	 * @param pred
	 * @return
	 */
	 ...
	 * @return
	 */
	public static <InType, OutType> Iterable<OutType> applyOneToManyTransform(final Iterable<InType> input,
	 ...
	 * @return
	 */
	public static <X,Y> Iterable<ZipPair<X,Y>> zip(final Iterable<X> iter1, final Iterable<Y> iter2) {
		return new ZipIterable<X,Y>(iter1, iter2);
	}

	public static <T> Iterable<List<T>> zip(final Iterable<? extends Iterable<? extends T>> iterables) {
				...
				public boolean hasNext() {
					if (iter1.hasNext() != iter2.hasNext()) {
						...
						throw new RuntimeException(
							String.format("Iterable length mismatch in zip: %s has %d elements, but the other has more.",
	...
	/**
	 * reduces an iterable to a single value starting from an initial value
	 * and applying down the iterable.
	 * @param iterable
	 * @param initial
	 * @param func
	 * @return
	...
	public static final Function<Iterator<?>, Boolean> HasNext = new Function<Iterator<?>, Boolean> () {
		@Override

...
import com.bbn.bue.common.strings.offsets.*;
 ...
 * * Class for storing and manipulating strings that have been read in
	 ...
	 *
	 * @author originally by David A. Herman, refactored by Edward Loper; translated to Java by Ryan Gabbard
 * @author rgabbard
 *
 */
public final class LocatedString {
	 ...
	 * Offset accessors
	 *********************************************************************/
	public EDTOffset startEDTOffset() { return bounds.startInclusive().edtOffset(); }
	public EDTOffset endEDTOffset() { return bounds.endInclusive().edtOffset(); }
	public CharOffset startCharOffset() { return bounds.startInclusive().charOffset(); }
	public CharOffset endCharOffset() { return bounds.endInclusive().charOffset(); }
	public OffsetGroupRange bounds() { return bounds; }

	public List<OffsetEntry> offsetEntries() { return offsets; }
	...
	public static LocatedString forString(final String text, final OffsetGroupRange bounds, final List<OffsetEntry> spanOffsets) {
	...
	public static LocatedString forString(final String text, final OffsetGroup initialOffsets, final boolean EDTOffsetsAreCharOffsets) {
		final List<OffsetEntry> offsets = calculateOffsets(text, initialOffsets, EDTOffsetsAreCharOffsets);
	...
	/**
	 * Returns the earliest offset group within this {@code LocatedString} whose character offset matches the one supplied.
	 *  If not such offset group exists,
	...
	public OffsetGroup offsetGroupForCharOffset(final CharOffset offset) {
		...
		for (final OffsetEntry entry : offsets) {
				...
				&& entry.endOffset.charOffset().value()>offset.value())
			{
				...
				return OffsetGroup.from(offset, new EDTOffset(entry.startOffset.edtOffset().value()+offsetWithinEntry));
			}
		}
	...
	private LocatedString(final String content, final List<OffsetEntry> offsets, final OffsetGroupRange bounds) {
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equal(this.bounds, other.bounds) && Objects.equal(this.content, other.content) &&  Objects.equal(this.offsets, other.offsets);
    }
    ...
    public static class OffsetEntry {
		...
		public OffsetEntry(final int startPos, final int endPos, final OffsetGroup startOffset, final OffsetGroup endOffset, final boolean isEDTSkipRegion) {
        ...
        public boolean equals(Object obj) {
            ...
            return Objects.equal(this.startPos, other.startPos) && Objects.equal(this.endPos, other.endPos) && Objects.equal(this.startOffset, other.startOffset) && Objects.equal(this.endOffset, other.endOffset) && Objects.equal(this.isEDTSkipRegion, other.isEDTSkipRegion);
        }
    }

	private static List<OffsetEntry> calculateOffsets(final String text, final OffsetGroup initialOffsets, final boolean EDTOffsetsAreCharOffsets) {
		checkNotNull(text);
		...
		for (; pos<text.length(); ++pos) {
			...
			if (!EDTOffsetsAreCharOffsets && pos > 0 &&
					...
					&& !(justLeftXMLTag && c=='<'))
			{
				final int prevEDTOffset = (edtOffset == 0 || prevChar == '\r') ? edtOffset : (edtOffset -1) ;
				offsets.add(new OffsetEntry(startPos, pos, start, OffsetGroup.from(new ByteOffset(byteOffset-1),
	...
	private static OffsetGroupRange boundsFromOffsets(final List<OffsetEntry> offsets) {
		...
		return OffsetGroupRange.from(offsets.get(0).startOffset, offsets.get(offsets.size() - 1).endOffset);
	}
	 ...
	 * @return
	 */
	private List<OffsetEntry> offsetsOfSubstring(final int startIndexInclusive, final int endIndexExclusive) {
		checkArgument(startIndexInclusive < endIndexExclusive,
				String.format("Start Index %d not less than end index %s", startIndexInclusive, endIndexExclusive));
		...
		for (int entryNum = entryBefore(startIndexInclusive); entryNum < offsets.size(); ++entryNum) {
			...
			final int charLength = entry.endOffset.charOffset().value() - entry.startOffset.charOffset().value();
			final int edtLength = entry.endOffset.edtOffset().value() - entry.startOffset.edtOffset().value();
			...
			if (entry.startPos < startIndexInclusive) {
				...
				newStartOffset = OffsetGroup.from(new CharOffset(startIndexInclusive), new EDTOffset(newEDTOffsetValue));
			}
			if (entry.endPos > endIndexExclusive) {
				...
				newEndOffset = OffsetGroup.from(new CharOffset(endIndexExclusive-1), new EDTOffset(newEDTOffsetValue));
			}
			newStartPos -= startIndexInclusive;
			newEndPos -= startIndexInclusive;
			ret.add(new OffsetEntry(newStartPos, newEndPos, newStartOffset, newEndOffset, entry.isEDTSkipRegion));
